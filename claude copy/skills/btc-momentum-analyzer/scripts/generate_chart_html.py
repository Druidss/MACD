...
from trading_signals import analyze_trading_signals, generate_trading_plan
...
def generate_trading_analysis(db: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # ==== 3日交易计划（基于4h/1h） ====
    # 优先检测4h级别信号，如无信号则检测1h级别
    day3_plan_4h = generate_trading_plan(db, '4h')
    day3_plan_1h = generate_trading_plan(db, '1h')

    # 优先使用4h级别信号（更可靠）
    if day3_plan_4h['direction'] in ['做多', '做空']:
        day3_plan = day3_plan_4h
        day3_plan['timeframe'] = '4h'
    elif day3_plan_1h['direction'] in ['做多', '做空']:
        day3_plan = day3_plan_1h
        day3_plan['timeframe'] = '1h'
    else:
        # 无信号，观望
        day3_plan = {
            'direction': '观望',
            ...
            'target1': None,
            'target2': None,
            'reason': f"等待买卖点信号 (4h DEA={tf_4h['dea']:.0f}, 价格距EMA52={(tf_4h['close']-tf_4h['ema52'])/tf_4h['ema52']*100:+.1f}%)",
            'buy_point_type': None
        }

    # ==== 1周交易计划（基于12h/1d） ====
    week_plan_12h = generate_trading_plan(db, '12h')
    week_plan_1d = generate_trading_plan(db, '1d')

    # 优先使用12h级别信号
    if week_plan_12h['direction'] in ['做多', '做空']:
        week_plan = week_plan_12h
        week_plan['timeframe'] = '12h'
        # 转换为entry_zone格式
        if week_plan['entry']:
            entry_low = week_plan['entry'] * 0.98
            entry_high = week_plan['entry'] * 1.02
            week_plan['entry_zone'] = f"{entry_low:,.0f}-{entry_high:,.0f}"
        else:
            week_plan['entry_zone'] = "等待信号"
    elif week_plan_1d['direction'] in ['做多', '做空']:
        week_plan = week_plan_1d
        week_plan['timeframe'] = '1d'
        if week_plan['entry']:
            entry_low = week_plan['entry'] * 0.98
            entry_high = week_plan['entry'] * 1.02
            week_plan['entry_zone'] = f"{entry_low:,.0f}-{entry_high:,.0f}"
        ...
        week_plan = {
            ...
            'target1': None,
            'target2': None,
            'reason': f"等待买卖点信号 (12h DEA={tf_12h['dea']:.0f}, 价格距EMA52={(tf_12h['close']-tf_12h['ema52'])/tf_12h['ema52']*100:+.1f}%)",
            'buy_point_type': None
        }

    # ==== 1月交易计划（基于2d） ====
    month_plan_2d = generate_trading_plan(db, '2d')

    if month_plan_2d['direction'] in ['做多', '做空']:
        month_plan = month_plan_2d
        month_plan['timeframe'] = '2d'
        month_plan['entry_condition'] = f"{month_plan_2d.get('buy_point_type', month_plan_2d.get('sell_point_type', '信号确认'))}"
        month_plan['target'] = month_plan['target2'] if month_plan['target2'] else month_plan['target1']
    else:
        # 无信号，观望
        if tf_2d['dea'] < -1000:
            direction = '等待'
            entry_condition = "2d DEA上穿0轴确认"
            reason = f"2d下跌线段，DEA={tf_2d['dea']:.0f}，等待变盘"
        else:
            direction = '谨慎观望'
            entry_condition = "等待买点信号"
            reason = f"2d 未出现明确买卖点 (DEA={tf_2d['dea']:.0f}, 价格距EMA52={(tf_2d['close']-tf_2d['ema52'])/tf_2d['ema52']*100:+.1f}%)"
...
<body>
    ...
    <div class="trading-plans">
        ...
        <div class="plan-card">
            ...
            <div class="plan-item">
                <div class="plan-label">进场位置 ({trading_analysis['day3'].get('timeframe', '4h')})</div>
                <div class="plan-value price">{f"{trading_analysis['day3']['entry']:.2f} USDT" if trading_analysis['day3']['entry'] else '等待信号'}</div>
            </div>
            <div class="plan-item">
                <div class="plan-label">止损位</div>
                <div class="plan-value">{trading_analysis['day3']['stop_loss']:.2f} USDT</div>
            </div>
            <div class="plan-item">
                <div class="plan-label">目标位 1</div>
                <div class="plan-value">{f"{trading_analysis['day3']['target1']:.2f} USDT" if trading_analysis['day3']['target1'] else '等待信号'}</div>
            </div>
            <div class="plan-item">
                <div class="plan-label">目标位 2</div>
                <div class="plan-value">{f"{trading_analysis['day3']['target2']:.2f} USDT" if trading_analysis['day3']['target2'] else '等待信号'}</div>
            </div>
            <div class="plan-reason">
                {f"[{trading_analysis['day3'].get('buy_point_type', '')}] " if trading_analysis['day3'].get('buy_point_type') else ''}{trading_analysis['day3']['reason']}
            </div>
        </div>
        ...
        <div class="plan-card">
            ...
            <div class="plan-item">
                <div class="plan-label">进场区间 ({trading_analysis['week'].get('timeframe', '12h')})</div>
                <div class="plan-value price">{trading_analysis['week']['entry_zone']}</div>
            </div>
            <div class="plan-item">
                <div class="plan-label">止损位</div>
                <div class="plan-value">{trading_analysis['week']['stop_loss']:.2f} USDT</div>
            </div>
            <div class="plan-item">
                <div class="plan-label">目标位 1</div>
                <div class="plan-value">{f"{trading_analysis['week']['target1']:.2f} USDT" if trading_analysis['week']['target1'] else '等待信号'}</div>
            </div>
            <div class="plan-item">
                <div class="plan-label">目标位 2</div>
                <div class="plan-value">{f"{trading_analysis['week']['target2']:.2f} USDT" if trading_analysis['week']['target2'] else '等待信号'}</div>
            </div>
            <div class="plan-reason">
                {f"[{trading_analysis['week'].get('buy_point_type', '')}] " if trading_analysis['week'].get('buy_point_type') else ''}{trading_analysis['week']['reason']}
            </div>
        </div>
        ...
        <div class="plan-card">
            ...
            <div class="plan-item">
                <div class="plan-label">进场条件 ({trading_analysis['month'].get('timeframe', '2d')})</div>
                <div class="plan-value">{trading_analysis['month']['entry_condition']}</div>
            </div>
            <div class="plan-item">
                <div class="plan-label">止损位</div>
                <div class="plan-value">{trading_analysis['month']['stop_loss']:.2f} USDT</div>
            </div>
            <div class="plan-item">
                <div class="plan-label">目标位</div>
                <div class="plan-value">{f"{trading_analysis['month']['target']:.2f} USDT" if trading_analysis['month']['target'] else '等待信号'}</div>
            </div>
            <div class="plan-reason">
                {f"[{trading_analysis['month'].get('buy_point_type', '')}] " if trading_analysis['month'].get('buy_point_type') else ''}{trading_analysis['month']['reason']}
            </div>
        </div>
    </div>
"""
