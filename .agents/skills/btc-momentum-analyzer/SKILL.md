...
**数据获取在本地机器运行（OKX API 可访问），分析在容器/Codex 中运行。**

```
本地机器（Mac）                     容器（Codex on the Web）
─────────────────                   ─────────────────────────────
...
cd /path/to/MACD
# 初始化数据库（下载历史数据）
python3 .Codex/skills/btc-momentum-analyzer/scripts/local_loop.py --init

# 验证数据
python3 .Codex/skills/btc-momentum-analyzer/scripts/database_manager.py --status
```
...
python3 .Codex/skills/btc-momentum-analyzer/scripts/local_loop.py --git-push

# 只更新一次（手动触发）
python3 .Codex/skills/btc-momentum-analyzer/scripts/local_loop.py --once --git-push

# 自定义间隔（如每 1 小时）
python3 .Codex/skills/btc-momentum-analyzer/scripts/local_loop.py --interval 60 --git-push

# 更新所有 8 个时间级别
python3 .Codex/skills/btc-momentum-analyzer/scripts/local_loop.py --all-timeframes --git-push
```
...
crontab -e
# 添加这行：
0 */6 * * * cd /path/to/MACD && python3 .Codex/skills/btc-momentum-analyzer/scripts/local_loop.py --once --git-push >> /tmp/btc_loop.log 2>&1
```
...
```bash
# 获取数据
python3 .Codex/skills/btc-momentum-analyzer/scripts/fetch_btc_data.py \
  --symbol BTC-USDT --timeframes 1d,12h,6h,4h --limit 200 \
  --output data/btc_latest.json

# 计算指标
python3 .Codex/skills/btc-momentum-analyzer/scripts/calculate_indicators.py \
...
| 容器（Codex on Web）| OKX/Binance API **403 被阻断** ❌ | 读 `btc_database.json` ✅ |
...
- **脚本位置**：`.Codex/skills/btc-momentum-analyzer/scripts/`
